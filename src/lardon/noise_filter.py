...
def median_filter_pds():
    ...
    if(0 < window <= cf.n_pds_stream_sample):
    ...
    if(0 < window <= cf.n_pds_trig_sample):
        
        """ Mask ROI with NaN for median computation """
        data_trig_masked = np.where(dc.mask_trig_pds, dc.data_trig_pds, np.nan)            
        """ Apply centered median filter """
        baseline = centered_median_filter(data_trig_masked, window)
